       ...
       y < (blockDim.y * gridDim.y)-pHeight-paddingY)
    {
        for(int j = -pHeight; j <= pHeight; j++)
        {
            ...
            {           
                ...
                //     sum = 0;
                // } 
                sum += float(source[((y+j) * width) + (x+i)]);   
            }
        }
    }
...
int main (int argc, char** argv)
{
    ...
    camera >> frame;
    unsigned char *sourceDataDevice, *blurredDataDevice, *edgesDataDevice, *thresholdDataDevice, *dilateDataDevice, *erodeDataDevice;
    ...
    while(1)
    {
        ...
        cudaEventRecord(start);
        {
            ...
            // theshould<<<pblocks,pthreads>>>(edgesDataDevice,thresholdDataDevice,170);

            Erode<<<cblocks,cthreads>>>(edgesDataDevice, frame.size().width, frame.size().height, 2, 2, erodeKernel, 5, 5, erodeDataDevice);
            Dilate<<<cblocks,cthreads>>>(erodeDataDevice, frame.size().width, frame.size().height, 2, 2, dilateKernel, 9, 9, dilateDataDevice);
